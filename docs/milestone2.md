...
* SwiftDiff implements elementary functions as AutoDiff object methods according to the rules of trignometric differentiation.
  * ```__add__```
  * ```__radd__```
  * ```__sub__```
  * ```__rsub__```
...
## To Implement
We have not yet implemented vector functionality for SwiftDiff. In order to accomodate more complex functions, multiple functions, and multiple inputs, we will use the following datastructures: tuples, lists, dictionaries, ndarray, and trees as core basic data structures. SwiftDiff custom classes will serve to facilitate the flow of data within the package.
Some examples:
